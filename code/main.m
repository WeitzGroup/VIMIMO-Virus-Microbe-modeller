...
if contains(dirstr,'cluster')
    ...
    err.vals = [];
    [~, ~, err.id, err.labels, err.legends] = mcmcmodel.ssfun(chain(transient_id,:),data);
    for i = 1:mcmcoptions.nsimu
        [~, err.vals(i,:)] = mcmcmodel.ssfun(chain(i,:),data);
    end
    clear i;

    %% Raunak's error bar implementation
    %either to be deleted or have to re write the new plotting functions.
    %out = mcmcpred(mcmcresults,chain,s2chain,data,mcmcmodel,mcmcoptions.nsimu,[]);
