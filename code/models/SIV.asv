classdef SIV < ode_funs
    ...
    methods
        ...
        function obj = SIV(NH,NV)
            ...
            obj.NH = NH;
            obj.NV = NV;
            % create an id for which state variable is which one
            %later will be useful to call them
            id.S = 1:NH;
        ...
        %create a vector to track all the state variables
        ...
        % probably at a particular time, sums up all S, I
        ...
        %at a particular time, sums up all the viruses
        function V = sum_viruses(obj,y) % free phage only
           V = y(:,obj.id.V); 
        end
        
        %actual differential equations (see page 10 of manuscript)
