classdef SEIVW < ode_funs
    ...
    methods
        ...
        function obj = SEIVW(NH,NV,NE)
            obj.name = sprintf('SEIVW%d',NE);
            obj.NH = NH;
            obj.NV = NV;
            obj.NE = NE;
            id.S = 1:NH;
            ...
            id.V = (1:NV)+id.I(end);
            id.W = (1:NV)+id.V(end); % non-infectious
            id.D = 1+id.W(end); % debris
        ...
        function V = sum_viruses(obj,y) % free phage only, including non-infectious particles
           V = y(:,obj.id.V) + y(:,obj.id.W); 
        end
        ...
        function dydt = ode(obj,t,y,pars)
            ...
            dV = (1-pars.q).*(pars.beta.*etaeff.*Imat)'*OH - V.*(viral_adsorb_fun(pars)'*N) - viral_decay_fun(pars,V);
            dW = pars.q.*(pars.beta.*etaeff.*Imat)'*OH - viral_decay_fun(pars,W);
            dD = sum(etaeff(:).*Imat(:)); % sum across all pairs for net lysis rate
            
            dydt = [dS; dEmat(:); dEmat2(:); dImat(:); dV; dW; dD];
